''''
为匹配ASA优化算法需要添加东西，因此稍微修改了一下tank模型'''
...
class TankModel(nn.Module):
    ...
    def set_parameters(self, params_dict):
        ...
        for name, value in params_dict.items():
            if hasattr(self, name):
                ...
                getattr(self, name).data.copy_(tensor_value)
    ...
    def get_parameters(self):
        ...
        for name, param in self.named_parameters():
            params_dict[name] = param.data.item()
        return params_dict
    ...
    def forward(self, packed_P, initial_S, Area, baseflow):
        ...
        for t in range(P.size(1)):
            ...
            # 存储流出量，考虑面积因素
            outflow_values.append((total_outflow * Area / 3.6 + baseflow * valid_mask))
