...
class NumDataset(Dataset):
    def __init__(self, dataset1 = 'train'):
        raindistrain = pd.read_csv(f'..\\SSIOWP-event\\{config.zhanming}\\raindistrain' + str(config.timesteps) + '_' + str(config.leadtimes) + '.csv',
                                   index_col=0)
        raindistest = pd.read_csv(f'..\\SSIOWP-event\\{config.zhanming}\\raindistest' + str(config.timesteps) + '_' + str(config.leadtimes) + '.csv',
                                  index_col=0)
        raindisval = pd.read_csv(f'..\\SSIOWP-event\\{config.zhanming}\\raindisval' + str(config.timesteps) + '_' + str(config.leadtimes) + '.csv',
                                 index_col=0)
        if dataset1 == 'train':
            rd = raindistrain
        if dataset1 == 'test':
