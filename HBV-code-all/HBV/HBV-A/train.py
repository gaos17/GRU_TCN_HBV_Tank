...
def train(model, epochs, patience = 10, ):
    ...
    for epoch in range(epochs):
        ...
        for index, (_, input, _, flowbase, area, target) in enumerate(train_dataloader):
            input = input.to(config.device)
            flowbase = flowbase.to(config.device)
            area = area.to(config.device)
            target = target.to(config.device)
            optimizer.zero_grad()
            decoder_outputs = model(config.ETP, input, config.initial_SM, config.initial_SUZ, config.initial_SLZ, area, flowbase)
        ...
        for index, (_, input, _, flowbase, area, target) in enumerate(val_dataloader):
            input = input.to(config.device)
            flowbase = flowbase.to(config.device)
            area = area.to(config.device)
            output = model(config.ETP, input, config.initial_SM, config.initial_SUZ, config.initial_SLZ, area, flowbase).cpu()
