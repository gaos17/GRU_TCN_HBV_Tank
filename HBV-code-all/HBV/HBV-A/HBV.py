...
class HBVModel(nn.Module):
    ...
    def gamma_unit_hydrograph(self, t, a, tau):
        # 伽马分布公式
        numerator = (t ** (a - 1)) * torch.exp(-t / tau)
    ...
    def forward(self, ETP, packed_P, initial_SM, initial_SUZ, initial_SLZ, Area, baseflow):
        ...
        gamma_weights_full = self.gamma_unit_hydrograph(time_steps_full, self.gamma_a, self.gamma_tau)  # Shape: (seq_length,)
        # 对伽马分布权重进行归一化
        ...
        for i in range(batch_size):
            ...
            # 初始化输出流量序列
            Q_convolved = torch.zeros(seq_length,device=config.device)  # Shape: (seq_length,)
            # 逐步累积计算卷积
        ...
        # 构造掩码，仅对有效序列长度进行计算
        mask = torch.arange(Q_weighted_tensor.size(1), device=config.device).unsqueeze(0) < lengths.unsqueeze(1)  # Shape: (batch_size, timesteps)
        mask = mask.float()  # 转换为浮点型，方便后续计算
        # 仅对有效部分加上基流，填充部分保持为 0
        Q_final = (Q_weighted_tensor * Area/3.6) * mask
