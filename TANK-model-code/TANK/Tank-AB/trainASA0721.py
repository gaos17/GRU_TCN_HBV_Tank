...
class AdaptiveSimulatedAnnealing:
    ...
    def evaluate(self, params, dataloader, dataset_type=None):
        ...
        for times, input, _, flowbase, area, target in dataloader:
            # ========== 关键修改 ========== #
            input_data = input.data.to(config.device)
            # 确保batch_sizes正确处理
            batch_sizes = input.batch_sizes[0]  # 直接取值，无需.item()
            if isinstance(batch_sizes, torch.Tensor) and batch_sizes.is_cuda:
